...
userSchema.methods.generateAccessToken = async function(user: string, id: string): Promise<any> { 
    ...
    const accessToken = await jwt.sign(
        { user, id }, accessSecret, {expiresIn: expireAccess}); //get the private key from the config file -> environment variable
    return accessToken;
}
...
userSchema.methods.generateRefreshToken = async function(user: string, id: string): Promise<any> { 
    ...
    const refreshToken = await jwt.sign(
        { user, id }, refreshSecret, {expiresIn: expireRefresh}); //get the private key from the config file -> environment variable
    return refreshToken;
}
...
userSchema.pre<IUser>('save', async function(next) {
    ...
    await bcrypt.genSalt(10, async function(err, salt) {
        ...
        // override the cleartext password with the hashed one
            // console.log(user.password, "PASSWORD FINAL")
